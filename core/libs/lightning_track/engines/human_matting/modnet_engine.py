...
class MODNetEngine(torch.nn.Module):
    def __init__(self, device='cpu'):
        ...
        for key in state_dict:
            clean_state_dict[key.replace('module.', '')] = state_dict[key]
        # build model
    ...
    def forward(self, input_image, return_type='matting', background_rgb=1.0):
        if input_image.max() > 2.0:
            warnings.warn('Image should be normalized to [0, 1]: {:.1f}'.format(input_image.max()))
        ...
        # padding
        if resized_h % 32 != 0 or resized_w % 32 != 0:
            image = torchvision.transforms.functional.pad(image, (32-resized_w % 32, 32-resized_h % 32, 0, 0, ), padding_mode='reflect')
        # normalize and forwarding
        image = self.normalize(image)[None]
        _, _, predict = self.model(image, True)
        predict = predict[0]
        # undo padding
